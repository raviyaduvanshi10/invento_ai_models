...
def get_predection_no(image,net,LABELS,COLORS):
    (H, W) = image.shape[:2]
...
wpath="weights/yolov3-tiny.weights"
tiny = True
...
def main():
    ...
    """data = get_predection_no(image,nets,Lables,Colors)
    #numberCount = 0
    for data1 in data:
        print("TeastValue",data1)"""
...
def main1():
    ...
    image=npimg.copy()
    image=cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
    res=get_predection_no(image,nets,Lables,Colors)
    #image=cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
    #image=cv2.cvtColor(res,cv2.COLOR_BGR2RGB)
    #image=cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
    # show the output image
    ...
    #img_encoded=image_to_byte_array(np_img)  
    #base64_bytes = base64.b64encode(img_encoded).decode("utf-8")    
    #return jsonify({"status": 200, "boxes": "res" }) 
...
def main2():
    ...
    _, img_encoded = cv2.imencode('.png', np.float32(np_img))
    response = img_encoded.tostring()

    try:
        return Response(response=response, status=200, mimetype='image/png')
        #return f'<img src="data:image/png;base64,{base64_bytes}">'
